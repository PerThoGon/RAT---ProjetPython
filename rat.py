...
def ipconfig(ssl_socket):
    ...
    if os_type == "posix":  # Test si la machine est une Linux
        conf = subprocess.run(['ip a'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)  # Exécution de la commande "ifconfig" et stockage de ses sorties
        ...
        conf = subprocess.run(['ip a'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)  # Exécution de la commande "ipconfig" et stockage de ses sorties
